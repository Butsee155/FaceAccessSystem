...
# ── CRITICAL: threshold based on your actual distances (1.2 - 1.7) ──────────
MATCH_THRESHOLD = 2.0

# ── CRITICAL: static_image_mode=True — must match admin_register.py ─────────
mp_face_mesh   = mp.solutions.face_mesh
mp_face_detect = mp.solutions.face_detection

face_mesh = mp_face_mesh.FaceMesh(
    static_image_mode=True,        # ← must be True
    max_num_faces=1,
...
def get_face_encoding(frame_bgr):
    """Identical to admin_register.py"""
...
def load_employees():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT EmployeeID, FullName, IDNumber, Department, 
               CompanyID, FaceEncoding 
        FROM Employees
    """)
...
def log_access(employee_id, granted):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO AccessLogs (EmployeeID, AccessGranted) VALUES (?, ?)",
...
class AccessTerminal:
    ...
    def build_ui(self):
        ...
        tk.Label(hdr, text="🔐  FACE ACCESS TERMINAL",
        ...
        tk.Label(hdr, textvariable=self.clock_var,
                 ...
                 fg=BLUE_LIGHT).pack(side="right", padx=25)
        tk.Button(hdr, text="⬅ Back to Login",
        ...
        # Info panel
        info_panel = tk.Frame(body, bg=BG_DARK, width=380)
        ...
        tk.Label(info_card, text="EMPLOYEE INFORMATION",
                 ...
                 bg=BG_CARD, fg=TEXT_GRAY).pack(anchor="w", padx=15, pady=(12, 5))
        ...
        # Distance debug label (helpful for tuning)
        self.dist_var = tk.StringVar(value="")
        tk.Label(info_panel, textvariable=self.dist_var,
                 font=("Segoe UI", 8), bg=BG_DARK,
                 fg=TEXT_GRAY).pack(anchor="w", pady=(0, 5))

        # Recent log table
        tk.Label(info_panel, text="RECENT ACCESS",
                 font=("Segoe UI", 8, "bold"),
                 bg=BG_DARK, fg=TEXT_GRAY).pack(anchor="w", pady=(5, 5))

        style = ttk.Style()
        style.configure("Term.Treeview",
                         background=BG_CARD, foreground=TEXT_WHITE,
                         fieldbackground=BG_CARD, rowheight=26,
                         font=("Segoe UI", 8))
        style.configure("Term.Treeview.Heading",
                         background=BG_PANEL, foreground=BLUE_LIGHT,
                         font=("Segoe UI", 8, "bold"))
        style.map("Term.Treeview", background=[("selected", BLUE_ACCENT)])

        cols = ("Name", "Time", "Status")
        self.log_tree = ttk.Treeview(info_panel, columns=cols,
                                      show="headings",
                                      style="Term.Treeview", height=6)
        for col, w in zip(cols, [150, 90, 80]):
    ...
    def update_frame(self):
        ...
        if ret and frame is not None:
            ...
            self.frame_skip += 1
            encoding = get_face_encoding(frame) if self.frame_skip % 2 == 0 else None

            now = datetime.now()

            if encoding is not None and len(self.employees) > 0:
                distances = [np.linalg.norm(encoding - e["encoding"])
                             for e in self.employees]
                best_idx  = np.argmin(distances)
                best_dist = distances[best_idx]

                # Show live distance for debugging
                self.dist_var.set(f"Best distance: {best_dist:.4f}  |  Threshold: {MATCH_THRESHOLD}")
                ...
                if best_dist < MATCH_THRESHOLD:
                    ...
                    self.status_text.config(text="ACCESS GRANTED",
                                             bg="#0A2E1A", fg=SUCCESS)

                    cv2.rectangle(frame, (5, 5), (535, 425), (0, 200, 100), 3)
                    ...
                    if time.time() - last > 5:
                        ...
                        self.add_log_entry(emp["full_name"],
                                           now.strftime("%H:%M:%S"), "GRANTED")

                else:
                    # Red status
                    self.status_frame.config(bg="#2E0A0A")
                    self.status_icon.config(text="❌", bg="#2E0A0A", fg=DANGER)
                    self.status_text.config(text="ACCESS DENIED",
                                             bg="#2E0A0A", fg=DANGER)
                    for key in self.info_vars:
                        self.info_vars[key].set("—")
                    cv2.rectangle(frame, (5, 5), (535, 425), (0, 0, 200), 3)

            elif encoding is None and self.frame_skip % 2 == 0:
                # No face
                self.status_frame.config(bg=BG_CARD)
                self.status_icon.config(text="👁", bg=BG_CARD, fg=TEXT_GRAY)
                self.status_text.config(text="Scanning...",
                                         bg=BG_CARD, fg=TEXT_GRAY)
                self.dist_var.set("")
